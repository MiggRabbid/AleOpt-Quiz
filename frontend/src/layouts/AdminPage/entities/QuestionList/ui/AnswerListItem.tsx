...
const AnswerListItem = (props: IAnswerListItemProps) => {
  ...
  return (
    <Box
      className={`flex gap-4 rounded-xl! px-4! py-4! ${isCorrect ? 'bg-green-200!' : 'bg-green-100!'}`}
    >
      <Typography
        className={`flex h-6! w-6! items-center justify-center rounded-full! text-xs! leading-none! font-bold! text-slate-800! uppercase! ${isCorrect ? 'bg-green-100!' : 'bg-green-200!'}`}
      >
        {answer.id}
      </Typography>
      <Typography
